...
const CardPhotos = ({ data, scale = 1 }) => {
  ...
  return (
    <>
      {scale === 2 && data ? (
        <div className="scale-110 transition-all text-white w-3/4 min-h-full overflow-hidden px-4 py-2 bg-[#FFBD5B] bg-opacity-40 rounded-md border-2 border-[#FFBD5B] hover:scale-125 md:scale-105 md:hover:scale-110" onClick={() => navigate(`/gallery/${data._id}`)}>
        ...
        <div className="hover:scale-105 transition-all text-white w-3/4 min-h-3/4 overflow-hidden px-4 py-2 bg-[#FFBD5B] bg-opacity-40 rounded-md border-2 border-[#FFBD5B]" onClick={() => navigate(`/gallery/${data._id}`)}>
